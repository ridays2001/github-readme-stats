...
const isValidGradient = colors => isValidHexColor(colors[1]) && isValidHexColor(colors[2]);
 ...
 */
const FlexLayout = ({ items, gap, direction }) => {
	// filter() for filtering out empty strings
	return items.filter(Boolean).map((item, i) => {
		let transform = `translate(${gap * i}, 0)`;
		if (direction === 'column') transform = `translate(0, ${gap * i})`;
